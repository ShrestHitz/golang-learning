...
<html lang="en">
  ...
  <body>
                      ...
                      <ul class="list-group">
                        <li class="list-group-item" :class="{ 'checked': todo.completed, 'not-checked': !todo.completed }" v-for="(todo, todoIndex) in todos" v-on:click="toggleTodo(todo, todoIndex)">
                            ...
                            <div class="btn-group float-right" role="group" aria-label="Basic example">
                              <button type="button" class="btn btn-success btn-sm custom-button" v-on:click.prevent.stop="editTodo(todo, todoIndex)"><span class="fa fa-edit"></span></button>
                              <button type="button" class="btn btn-danger btn-sm custom-button" v-on:click.prevent.stop="deleteTodo(todo, todoIndex)"><span class="fa fa-trash"></span></button>
                            </div>
                        </li>
                      </ul>
                  </div>
    ...
    <script type="text/javascript">
      // Fix 1: renamed var from "Vue" to "app" to avoid overwriting the Vue constructor
      var app = new Vue({
        el: '#root',
        delimiters: ['@{', '}'],
        data: {
        ...
        mounted() {
          // Fix 5: added leading slash to all API URLs
        ...
        methods: {
          addTodo() {
            if (this.todo.title == '') {
              ...
              if (this.enableEdit) {
                // Fix 2: reset todo INSIDE .then(), after server confirms success
                this.$http.put('/todo/' + this.todo.id, this.todo).then(response => {
                  if (response.status == 200) {
                    // Fix 3: use this.$set() for reactive array update
                    this.$set(this.todos, this.todo.todoIndex, {...this.todo});
          ...
          toggleTodo(todo, todoIndex) {
            // Minor: simplified toggle logic
            var completedToggle = !todo.completed;
            this.$http.put('/todo/' + todo.id, {id: todo.id, title: todo.title, completed: completedToggle}).then(response => {
              if (response.status == 200) {
                // Fix 3: use this.$set() for reactive nested property update
                this.$set(this.todos[todoIndex], 'completed', completedToggle);
              }
            });
          },
          editTodo(todo, todoIndex) {
            this.enableEdit = true;
            // Fix 4: clone the object so edits don't mutate the list item live
            this.todo = {...todo, todoIndex: todoIndex};
          },
